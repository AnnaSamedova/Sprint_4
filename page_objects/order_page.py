...
class OrderPageScooter(BasePage):
    ...
    button_order_two = (By.XPATH, './/div[@class = "Home_FinishButton__1_cWm"]/button[text() = "Заказать"]')
    ...
    select_metro_station = (By.XPATH, './/input[contains(@placeholder, "* Станция метро")]')
    list_metro_station = (By.XPATH, './/ul[contains(@class, "select-search__options")]')
    ...
    button_order_scooter = (By.XPATH, './/div[@class = "Order_Buttons__1xGrp"]/button[text() = "Заказать"]')
    popup_agreement = (By.XPATH, './/div[@class = "Order_Buttons__1xGrp"]/button[text() = "Да"]')
    popup_order_is_processed_text = (By.XPATH, './/div[contains(@class, "Order_Text__2broi")]')
    button_check_status_order = (By.XPATH, './/button[contains(text(), "Посмотреть статус")]')
    ...
    @allure.step('Заполнить все поля первой части анкеты - имя, фамилия, адрес, метро, номер телефона, нажать кнопку Далее')
    def set_first_part_rental_form(self, name, surname, address, phone):
        ...
        self.click_on_any_element(OrderPageScooter.button_next)
